...
const LandingPage = ({ onLogin }) => {
  ...
  const [showPersonalMessage, setShowPersonalMessage] = useState(false); // State for Personal hover
  const [showBusinessMessage, setShowBusinessMessage] = useState(false); // State for Business hover
  const [showContactUsMessage, setShowContactUsMessage] = useState(false); // State for Contact Us hover
  const [showAboutUsMessage, setShowAboutUsMessage] = useState(false); // State for About Us hover
      ...
      <style>
        {`
          ...
          .landing-container::before {
            ...
            width: 100%;
            height: 100%;
            background: rgba(0, 0, 0, 0.2); /* Reduced opacity for transparency */
            z-index: 1;
          }
          ...
          .nav-button:hover {
            color: #1abc9c; /* Change color on hover instead of background */
          }
          ...
          .landing-content.content-with-margin {
            margin-top: 150px; /* Adjusted height to accommodate hover messages */
          }
          ...
          /* Existing styles for auth forms */
          .auth-container {
            display: flex;
            justify-content: center;
            align-items: center;
          ...
          /* Styles for the centered buttons */
          .button-box {
          ...
          .hover-message {
            ...
            text-align: left;
            margin-top: 10px; /* Small gap from button */
          }

          /* Adjust landing-content when hover is active */
          .landing-container.hover-active .landing-content {
            margin-top: 150px; /* Push content down when hover message is shown */
          }
          ...
          .card {
            ...
            position: relative;
            color: #333;
            min-height: 300px; /* Added to ensure uniform height */
          }
          ...
          /* Ensure landing-content adjusts for new layout */
          .landing-content {
            display: flex;
            flex-direction: column;
            justify-content: center;
          ...
          /* Ensure landing-container takes full height for footer positioning */
          .landing-container {
            min-height: 100vh;
            display: flex;
            flex-direction: column;
